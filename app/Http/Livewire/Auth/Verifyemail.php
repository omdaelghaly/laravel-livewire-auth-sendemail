...
{
    public $code, $token,$email,$status,$codestatus 
    //,$d,$h,$i
    ,$name,$setexpiretime, $expire ,$usercode;
...
//    $this->i =date("i");
  ...
  //Mail::to($this->email)->send(new Verifye($this->code,$this->token,$this->name));
...
//// verify emal with code buttn

   public function verifyemail()
   {
    $id= auth()->user()->id;
    $user = User::find($id); 
    $userv = Checkemail::where('user_id',$id)->first();
    if($userv){
            $code=$userv->code;
            $usercode=$this->usercode;
            if($usercode == $code){
                $this->codestatus = ' تم التفعيل  ';  
                ...
                $this->codestatus = 'هناك خطا فى الكود   ';               
            }
    }else{
        $this->codestatus = '  لم يتم ارسال الكود قم بارسال الكود اولا   ';
    }
